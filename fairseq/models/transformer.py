...
class TransformerModel(FairseqEncoderDecoderModel):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--first-read', type=int, metavar='D', default=2,
                            help='down bound')
        parser.add_argument('--cands-per-token', type=int, metavar='D', default=3,
                            help='cands_per_token')
        parser.add_argument('--curriculum-update', type=int, metavar='D', default=160000,
                            help='curriculum_update')
        parser.add_argument('--min-future', type=float, metavar='D', default=0.05,
                            help='min_future')
        # fmt: on
...
class TransformerDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        # Parma. for HMT
        self.first_read = getattr(args, "first_read", 2)
        self.cands_per_token = getattr(args, "cands_per_token", 3)
        self.transition_proj = nn.Linear(2 * embed_dim, 2, bias=False)
    ...
    def forward(
        ...
        src_hidden = encoder_out[0].transpose(0, 1)
    ...
    def extract_features_scriptable(
        ...
        if valid:
            ...
            ratio = 1 - update_num / self.curriculum_update
            if ratio < self.min_future:
                ratio = self.min_future
        src_len = encoder_out.encoder_out.size(0)
        
        cands = self.bulid_cands(
            x.size(1), self.first_read + max(int((src_len - self.first_read) * ratio), 0), self.cands_per_token
        ).clamp(1, encoder_out[0].size(0))
